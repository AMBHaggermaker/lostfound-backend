...
router.get('/', async (req, res, next) => {
  try {
    ...
    for (const tip of tips) {
      ...
      const pinnedPrefix = tip.is_pinned ? '📌 PINNED  ' : '';
      const displayName = tip.submitter_display?.trim() || 'Anonymous';

      doc.rect(50, doc.y, 495, 18).fill(tip.is_pinned ? '#fff4e0' : '#f5f5f5');
      doc.fillColor('#333').fontSize(9).text(
        `${pinnedPrefix}${label}${verifiedSuffix}  —  ${displayName}  —  ${new Date(tip.created_at).toLocaleDateString()}`,
        55, doc.y - 14, { width: 485 }
      );
      doc.moveDown(0.6);

      const tipLoc = tip.location_description || tip.location;
      if (tipLoc || tip.occurred_at) {
        doc.fontSize(9).fillColor('#666').text(
          [tipLoc && `Location: ${tipLoc}`, tip.occurred_at && `When: ${new Date(tip.occurred_at).toLocaleDateString()}`]
            .filter(Boolean).join('   ')
        );
        doc.moveDown(0.2);
      }
    ...
    doc.fontSize(8).fillColor('#999').text(
      `This document was generated by Mycelium Lost & Found on ${new Date().toUTCString()}.`,
      { align: 'center' }
    );
    doc.fontSize(8).fillColor('#999').text(`${BASE_URL}/cases/${lostCase.id}`, { align: 'center' });
